...
- **自動分類** — 受信メールを 8 種類のタグ（業務・緊急・迷惑・返信必要など）に自動分類し、即時返信の要否も判定する
- **本文要約** — 長文メールを簡潔に要約し、ダッシュボード上でひと目で内容を把握できる
- **AI 返信文案生成** — メール内容に応じた返信文を LLM が自動生成。テンプレートと組み合わせてそのまま送信可能
- **返信テンプレート管理** — クライアント別・用途別に複数の返信テンプレートを保存・選択可能。返信フォームのドロップダウンから選んで本文に挿入できる
- **返信ステータス管理** — 返信を送信すると「返信必要」タグが「返信済み」に自動更新され、対応漏れを防げる
- **新規メール作成・送信** — `/mails/compose` から新規メールを作成・送信可能。Google People API と連携した宛先候補のオートコンプリート（連絡先チップ選択）に対応
- **添付ファイル表示・ダウンロード** — 添付ファイルの一覧をメール詳細画面に表示し、認証付きダウンロードに対応。ファイルサイズも表示される
- **期限抽出・カレンダー連携** — 本文中に含まれる期限・日時を抽出し、Google Calendar に自動登録する
- **TODO 管理** — 対応が必要と判断されたメールは TODO として自動登録。タスクボード（ドラッグ&ドロップでステータス変更）、優先度（高 / 中 / 低）設定、完了タスクの削除に対応
- **音声入力** — 返信フォームでマイクから音声入力し、文字起こしして返信文に反映できる
- **ダッシュボードの検索・一括削除** — キーワード・タグ・返信要否・日付での絞り込み検索、チェックボックスによるメールの一括削除
- **Slack 通知** — 要返信メールを検出すると、設定した Slack チャンネルへ自動で通知する
- **週次メールサマリーレポート** — 直近 7 日間の受信数・要返信数・返信済み数・TODO 生成数などを毎週月曜 9:00 に自分の Gmail 宛へ自動送信
- **定期パイプライン** — APScheduler によるバックグラウンド自動実行（5 分〜1 時間、または無効 から選択）で、手動操作なしにメールが常に最新の状態に保たれる
- **前処理パイプライン** — HTML 除去・署名カット・URL 匿名化など 7 段階のクリーニングを経てから LLM に入力するため、ノイズを排除した高精度な分析が可能
